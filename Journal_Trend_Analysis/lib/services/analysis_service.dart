...
class JournalInfo {
  ...
  factory JournalInfo.fromJson(Map<String, dynamic> json) => JournalInfo(
    ...
    paperCount: json['paperCount'] as int? ?? 0,
    totalCitations: json['totalCitations'] as int? ?? 0,
...
class AnalysisService {
  ...
  Future<KeywordTrend> getTopicTrend(
    ...
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/analysis/topic-trend',
  ...
  Future<List<KeywordTrend>> compareKeywords(
    ...
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/analysis/compare-keywords',
    );
    final response = await _client
  ...
  Future<List<EmergingTopic>> getEmergingTopics({int take = 10}) async {
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/analysis/emerging-topics',
    ...
    if (response.statusCode != 200) {
      throw ApiError(
        'Failed to load emerging topics',
        statusCode: response.statusCode,
      );
    }
  ...
  Future<List<AuthorInfo>> getTopAuthors({int take = 10}) async {
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/authors/top',
    ).replace(queryParameters: {'take': take.toString()});
    final response = await _client
        .get(uri, headers: await _headers())
        .timeout(AppConfig.httpTimeout);
    ...
    return list
        .map(
          (e) => JournalInfo(
            ...
            name: e['name']?.toString() ?? '',
            paperCount: e['paperCount'] as int? ?? 0,
            totalCitations: e['totalCitations'] as int? ?? 0,
            avgCitations: (e['avgCitations'] as num?)?.toDouble() ?? 0,
          ),
        )
        .toList();
  }
  ...
  Future<List<dynamic>> getSyncLogs({int take = 50}) async {
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/admin/sync/logs',
    ).replace(queryParameters: {'take': take.toString()});
    final response = await _client
        .get(uri, headers: await _headers())
        .timeout(AppConfig.httpTimeout);
  ...
  Future<void> banUser(String userId) async {
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/admin/users/$userId/ban',
    );
    final response = await _client
  ...
  Future<void> unbanUser(String userId) async {
    final uri = Uri.parse(
      '${AppConfig.apiBaseUrl}/api/admin/users/$userId/unban',
    );
    final response = await _client
        .put(uri, headers: await _headers())
        .timeout(AppConfig.httpTimeout);
